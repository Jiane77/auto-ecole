...
class Modele{
    ...
    private $mdp;
     public function __construct() {
        ...
        $this->url  = "mysql:host={$env['DB_HOST']};dbname={$env['DB_NAME']};charset=utf8";
        $this->user = $env['DB_USER'];
        $this->mdp  = $env['DB_PASS'];
...
public function insert_moniteur($moniteur){
    ...
    $donnees=[
        ...
        ":idvehicule"=>$moniteur['idvehicule']??null
    ];
...
public function selectWhere_moniteur($idmoniteur){
    ...
    $exec=$this->unPdo->prepare($requete);
    $exec->execute([":idmoniteur"=>$idmoniteur]);
...
public function selectAll_moniteur(){
    ...
    public function selectLike_moniteur($mot){
        $requete = "SELECT * FROM moniteur 
                WHERE nommoniteur LIKE :mot OR prenommoniteur LIKE :mot";
        $exec = $this->unPdo->prepare($requete);
    ...
    public function update_moniteur($moniteur){
        ...
        ";
         $donnees=[
            ...
            ":idmoniteur" => $moniteur['idmoniteur']??null
        ];
        $exec=$this->unPdo->prepare($requete);
        $exec->execute($donnees);
    }
...
public function selectWhere_vehicule($idvehicule){
    $requete ="select *from vehicule where idvehicule=:idvehicule";
    $exec=$this->unPdo->prepare($requete);
    $exec->execute([":idvehicule"=>$idvehicule]);
    return $exec->fetch(PDO::FETCH_ASSOC);
}
